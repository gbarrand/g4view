      ...
      && (theta - (fSPhi + fDPhi) <=  halfAngTolerance) )
    {
      ...
      if ( IsDistanceToIn == true )
      {
        if (std::fabs(t) < halfCarTolerance )
        {
          ...
          scal = v* G4ThreeVector( p.x()*(1-fRtor/std_hypot(p.x(),p.y())),
                                   p.y()*(1-fRtor/std_hypot(p.x(),p.y())),
                                   p.z() );
...
G4ThreeVector G4Torus::SurfaceNormal( const G4ThreeVector& p ) const
{
  ...
  rho = std_hypot(p.x(),p.y());
  pt  = std_hypot(p.z(),rho-fRtor);
                                       ...
                                       G4ThreeVector  *n  ) const
{
  ...
  G4double rho = std_hypot(p.x(),p.y());
  G4double pt = std_hypot(p.z(),rho-fRtor);
