    ...
    for ( i = 0; i < ( tupleCount & 0xFFF ); ++i )
    {
      ...
      else if ( localpoints == ALL_POINTS )
      {
        ...
        for ( j = 0; j < face->cvt_size; ++j )
          face->cvt[j] += (FT_Short)FT_MulFix( deltas[j], apply );
      }

      else
      {
        for ( j = 0; j < point_count; ++j )
          face->cvt[localpoints[j]] += (FT_Short)FT_MulFix( deltas[j], apply );
      }
...
/*G.Barrand :
    ...
    if (FT_NEW_ARRAY( delta_xy, n_points )) goto Exit;
