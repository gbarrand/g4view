...
static int gl_tab(char *buf, int offset, int *loc,void* tag); /*G.Barrand : add void* tag.*/
...
int             (*Gl_tab_hook)(char *buf, int prompt_width, int *loc,void* tag) = gl_tab; /*G.Barrand : add void* tag.*/
int             (*Gl_in_key)(int ch) = 0;
void*           Gl_tab_tag = 0;  /*G.Barrand.*/
...
/* G.Barrand : if compiling with CL.exe, but running from a CYGWIN or WSL prompt,
               the Windows console logic does not work. In this case we attempt to use stdin
               to input commands. Note that in this case the below UNIX is_there_stdin_input()
               does not work because stdin is not a socket with Windows/VisualC++.
               Note that in this case isatty(stdin) is 0, but we enforce "gl_notty = 0"
               in gl_init() since it seems to work as for UNIX.
*/
static int gl_is_windows_console = 0; /*G.Barrand : 1 use windows console. 0 use stdin/stdout/stderr.*/
#else
...
#if defined(TIOCGETP) && !defined(__sgi) && !defined(R__PPCLINUX) && !defined(R__ALPHALINUX)
/* use BSD interface if possible */
#include <sgtty.h>
struct sgttyb   new_tty, old_tty;
struct tchars   tch;
struct ltchars  ltch;
#else
#ifdef SIGTSTP          /* need POSIX interface to handle SUSP */ /*__APPLE__, ubuntu, centos*/
#include <termios.h>
#if defined(__sun) || defined(__sgi) || defined(R__PPCLINUX) || defined(R__ALPHALINUX)
...
#ifdef _WIN32
int Gl_windows_console_getchar_()
{
...
static void flush_unit(int a_id) {
  _commit(a_id);
  /*fsync(a_id);*/
}
#endif
...
# define k_beep     7
  ...
  } else { /*!gl_is_windows_console*/
    unsigned char ch;
    flush_unit(1); /*G.Barrand*/
    flush_unit(2); /*G.Barrand*/
    while ((c = (read(0, &ch, 1) > 0) ? ch : -1) == -1 && errno == EINTR) errno = 0;
    /*
    printf("debug : %d\n",c);
    fflush(stdout);
    */
...
/*G.Barrand : begin :*/    
    ...
    if (isatty(0) == 0 || isatty(1) == 0) { /*stdin not a tty. We assume that we run from a CYGWIN or WSL prompt.*/
      gl_is_windows_console = 0;
      gl_notty = 0; /*Set it anyway to 0, since read(), write() seem to work as if on UNIX.*/
    } else { /*We assume that we run from a DOS prompt.*/
      gl_is_windows_console = 1;
      gl_notty = 0;
    }
#else    
    if (isatty(0) == 0 || isatty(1) == 0) gl_notty = 1;
#endif
/*G.Barrand : end.*/    
    gl_char_init();
    gl_init_done = 1;
    ...
    while ((c = gl_getc()) >= 0) {
        ...
        if (c >= ' ') {
            ...
            case '\t':                                        /* TAB */
                 if (Gl_tab_hook) {
                      tmp = gl_pos;
                      loc = Gl_tab_hook(gl_buf, strlen(gl_prompt), &tmp,Gl_tab_tag); /*G.Barrand : pass Gl_tab_tag.*/
            ...
            case '\033': /*dec=27*/
                 ...
                 case 'O':                                /* xterm arrow keys */
                      ...
                      case 'B':  /*66*/                        /* down */
                           ...
                           gl_fixup(gl_prompt, 0, BUF_SIZE);
                           break;
                      case 'C': gl_fixup(gl_prompt, -1, gl_pos+1);  /*67*/ /* right */
                           break;
                      case 'D': gl_fixup(gl_prompt, -1, gl_pos-1);  /*68*/ /* left */
                           break;
...
static int
gl_tab(char *buf, int offset, int *loc,void* tag)  /*G.Barrand : add void*.*/
...
/* G.Barrand */
int Gl_is_windows_console() {
#ifdef _WIN32
  gl_init();
  if(gl_is_windows_console) return 1;
#endif  
  return 0;
}
...
int Gl_is_there_stdin_input(int* a_is) {
#ifdef _WIN32
  if(gl_is_windows_console) {
    static HANDLE hConsoleInput = NULL;
    DWORD cRead;
    ...
    if(!hConsoleInput) hConsoleInput = GetStdHandle(STD_INPUT_HANDLE);
    if((hConsoleInput==NULL)||(hConsoleInput==INVALID_HANDLE_VALUE)) {*a_is = 0;return 0;}
    //FIXME : Have a "wait for" console input ? (from MSDN ReadConsoleInput)
    if(!PeekConsoleInput(hConsoleInput,&pirBuffer,1,&cRead)) {*a_is = 0;return 0;}    
    if(!cRead) {*a_is = 0;return 1;}
    // Should correspond to what is done in Getline.c/pause function for _WIN32 :
    ...
    /* How to query stdin ? (which is not a socket with Windows/VisualC++.)*/
    *a_is=0;
    return 1;
  }
#else
#define MAXIMUM(a,b) ((a)>(b)?(a):(b))
  ...
  fd_in = fileno(stdin);   //_WIN32 : stdin is not a socket.
  ...
  nfds++;
  if(select(nfds,&mask,0,0,&timeout)==(-1)) {*a_is = 0;return 0;}

  *a_is = FD_ISSET(fd_in,&mask)?1:0;
