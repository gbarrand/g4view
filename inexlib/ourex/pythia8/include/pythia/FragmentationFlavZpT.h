...
public:
  ...
  //G.Barrand : OSX : 10.6 : if compiling with -x objective-c++
  FlavContainer& operator=(const FlavContainer& flav) { if (this != &flav) { 
    this->id = flav.id; rank = flav.rank; nPop = flav.nPop; idPop = flav.idPop;
    idVtx = flav.idVtx; } return *this; }

  // Invert flavour.
  FlavContainer& anti() {this->id = -this->id; return *this;}
