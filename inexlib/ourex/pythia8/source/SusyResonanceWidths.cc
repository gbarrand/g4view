...
const bool SUSYResonanceWidths::s_DEBUG = false;
...
bool SUSYResonanceWidths::init(Info* infoPtrIn, Settings* settingsPtrIn,
  ...
  if(hasDecayTable && settingsPtr->flag("SLHA:useDecayTable")) {
    ...
    if(s_DEBUG) cout<<"Found decay table for:"<<idRes<<endl;  
  }
...
void ResonanceSquark::calcWidth(bool) {
  ...
  if(ps == 0.) return;
    ...
    widNow = fac * wid * ps;
    if(s_DEBUG) cout<<idRes<<":: id1:"<<id1Abs<<" id2:"<<id2Abs
		  <<" Width: "<<widNow<<endl;
    return;
  }
